...
2. SignUpModal opens
   ↓
3. User enters email/password OR clicks Google OAuth
   ↓
4. supabase.auth.signUp() or supabase.auth.signInWithOAuth()
   ↓
5. Supabase creates user in auth.users table
   ↓
6. Frontend: onAuthStateChange listener fires (event="SIGNED_IN")
   ↓
7. Frontend: Retrieve anonymous_session_id from localStorage
   ↓
8. Frontend: fetch(CLAIM_WEBHOOK_URL, { body: { anonymous_session_id, user_id } })
   ↓
9. Backend Auth Step 2 (validate_request): Parse and validate UUIDs
   ↓
10. Backend Auth Step 3 (claim_anonymous_generations):
    ...
    - Count affected rows
    ↓
11. Backend Auth Step 4: Return { success: true, claimed: { videos, generations } }
    ↓
12. Frontend: localStorage.removeItem("river_session_id")
    ↓
13. Frontend: fetchUserGenerations(userId)
    - Query Supabase for all user's generations
    - Open UserDashboard with history
```
